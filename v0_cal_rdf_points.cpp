...
const double r0= 0.1; // smallest sphere radius of rdf
...
void cal_dis_i(double dis[N_totbox], int a1, int b1, int c1, int a2, int b2, int c2);
//######################### functions #########################//
...
void cal_rdf(double rho){
	...
	int sum[nr]= {0}; // sum of all ltcp included within a certain shell
	...
	for(int a=0; a < nr; a++){
		...
		fprintf(out, "%f %f\n", r_print, (double) sum[a] / nall);
		fprintf(out_nor, "%f %f\n", r_print, (double) sum[a] / nall / volume / rho);
	}
}
